...
import { Calendar, Compass, ShieldCheck, DollarSign, CheckCircle2, Flame, MapPin } from 'lucide-react';

export function Header({ itinerary, checklist, budget, onResetData }) {
  // Calculate countdown to Sept 22, 2026
  const targetDate = new Date('2026-09-22T08:00:00');
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-semibold tracking-wide uppercase">
            ...
            <span>22 a 28 de Setembro de 2026 • 7 Dias / 6 Noites</span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-6">
          {/* Countdown Card */}
          <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-amber-500/40 transition-all group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-slate-400">Contagem Regressiva</span>
            ...
            <div className="mt-3">
              <div className="text-2xl sm:text-3xl font-bold text-white flex items-baseline gap-1">
                {diffDays} <span className="text-xs font-normal text-slate-400">dias</span>
              </div>
              <span className="text-[11px] text-amber-400/90 font-medium">Início: 22/09/2026</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-emerald-500/40 transition-all group">
            ...
            <div className="mt-3">
              <div className="text-2xl sm:text-3xl font-bold text-white flex items-baseline gap-1">
                {completedActivities} <span className="text-xs font-normal text-slate-400">/ {totalActivities}</span>
              </div>
          ...
          <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-orange-500/40 transition-all group">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-slate-400">Orçamento Real vs Teto</span>
